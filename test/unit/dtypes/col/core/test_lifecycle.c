...
void test_col_create_array() {
    ...
    /* valid */
    double *double_data = col_double_data_create(999);
    struct col *col_double = col_double_dummy_create("double", 999);
    col_assert(col_double, "double", double_data, 999, COL_DTYPE_DOUBLE, err);
    free(double_data);
    col_free(col_double);

    float *float_data = col_float_data_create(999);
    struct col *col_float = col_float_dummy_create("float", 999);
    col_assert(col_float, "float", float_data, 999, COL_DTYPE_FLOAT, err);
    free(float_data);
    col_free(col_float);

    int64_t *int64_data = col_int64_data_create(999);
    struct col *col_int64 = col_int64_dummy_create("int64", 999);
    col_assert(col_int64, "int64", int64_data, 999, COL_DTYPE_INT64, err);
    free(int64_data);
    col_free(col_int64);

    int32_t *int32_data = col_int32_data_create(999);
    struct col *col_int32 = col_int32_dummy_create("int32", 999);
    col_assert(col_int32, "int32", int32_data, 999, COL_DTYPE_INT32, err);
    free(int32_data);
    col_free(col_int32);

    uint8_t *uint8_data = col_uint8_data_create(999);
    struct col *col_uint8 = col_uint8_dummy_create("uint8", 999);
    col_assert(col_uint8, "uint8", uint8_data, 999, COL_DTYPE_UINT8, err);
    free(uint8_data);
    col_free(col_uint8);

    char **string_data = col_string_data_create(999);
    struct col *col_string = col_string_dummy_create("char", 999);
    col_assert(col_string, "char", string_data, 999, COL_DTYPE_STRING, err);
    ...
    /* err */
    double *data = col_double_data_create(999);

    struct col *col_null_name = col_create_array(NULL, data, 999, COL_DTYPE_DOUBLE, &err);
    assert(col_null_name == NULL);
    assert(err == COL_ERR_EMPTY_NAME);

    struct col *col_blank_name = col_create_array("", data, 999, COL_DTYPE_DOUBLE, &err);
    assert(col_blank_name == NULL);
    assert(err == COL_ERR_EMPTY_NAME);

    struct col *col_empty_data = col_create_array("no_data", NULL, 999, COL_DTYPE_DOUBLE, &err);
...
void test_col_clone() {
    ...
    char **string_data = col_string_data_create(999);
    struct col *col_string = col_string_dummy_create("string", 999);
    ...
    /* err */
    struct col *col_valid1 = col_double_dummy_create("valid1", 999);
...
void test_col_free() {
    struct col *col = col_double_dummy_create("col", 999);
